...
-- Updated command to list devices using PipeWire
local LIST_DEVICES_CMD = [[sh -c "pw-cli list-objects | grep -E 'node.name|media.class'"]]

-- Updated function to get volume using wpctl
local function GET_VOLUME_CMD()
  return "wpctl get-volume @DEFAULT_AUDIO_SINK@"
end

-- Updated function to increase volume using wpctl
local function INC_VOLUME_CMD(step)
  return "wpctl set-volume @DEFAULT_AUDIO_SINK@ " .. step .. "%+"
end

-- Updated function to decrease volume using wpctl
local function DEC_VOLUME_CMD(step)
  return "wpctl set-volume @DEFAULT_AUDIO_SINK@ " .. step .. "%-"
end

-- Updated function to toggle mute using wpctl
...
local function build_rows(devices, on_checkbox_click, device_type)
  ...
  for _, device in pairs(devices) do
    ...
    checkbox:connect_signal("button::press", function()
      spawn.easy_async(string.format([[sh -c 'wpctl set-default %s "%s"']], device_type, device.name), function()
        on_checkbox_click()
      end)
    end)
...
local function worker(user_args)
  ...
  function volume:toggle()
    if toggle_cmd == nil then
      ...
      spawn.easy_async(toggle_cmd, function()
